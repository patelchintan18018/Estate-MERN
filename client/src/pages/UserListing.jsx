        ...
        <>
          <Swiper navigation={{prevEl: false, nextEl: false}} loop={true} autoplay={{delay: 2500}} speed={1000}>
            {listing.imageUrls.map((url) => (
              <SwiperSlide key={url}>
                <div className=" h-[450px] sm:h-[550px] w-full" style={{background:`url(${url}) center no-repeat `, backgroundSize: 'cover', backgroundPosition: 'center'}}></div>
              </SwiperSlide>
            ))}
          </Swiper>
          <div className="flex flex-col gap-4 max-w-5xl mx-auto p-3 my-7">
            <p className="text-2xl font-bold text-slate-800"><span className="uppercase">{listing.name}</span> - ${' '}{listing.offer ? listing.regularPrice - listing.discountPrice: listing.regularPrice} {listing.type === 'rent' ? '/ Month' : ''} {listing.offer && (<span className="text-sm">($ {listing.discountPrice} OFF)</span>)}</p>
            ...
            <div className="flex gap-6 flex-wrap">
              ...
              <p className="flex gap-2 items-center mr-4"><FaChair className="text-green-700 text-xl"/>{listing.furnished ? 'Furnished': 'Not furnished'}</p>
            </div>
